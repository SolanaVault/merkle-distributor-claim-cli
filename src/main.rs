...
fn main() -> Result<()> {
    ...
    let merkle_distributor_program_id =
        Pubkey::from_str("MRKGLMizK9XSTaD1d1jbVkdHZbQVCSnPpYiTw9aKQv8")?;
    dotenv().ok();
...
fn create_token_account(
    ...
    let token_account =
        spl_associated_token_account::get_associated_token_address(&sdk_to_spl(user_key), &mint);
    let token_account_info = client.get_account(&spl_to_sdk(&token_account));
    if token_account_info.is_ok() {
        return Ok(None);
    }
    let spl_create_account_ix =
...
fn check_airdrop(
    ...
    let url = format!("{}/{}.json", base_url, user_key);
    ...
    if response.status().is_success() {
        ...
        if account.is_ok() {
            ...
            println!(
                "{:?} point Airdrop is available to claim.",
                (proof.amount as f64) / 1_000_000f64
            );
            return Ok(proof);
        }
...
fn claim_airdrop(
    ...
    let to = spl_associated_token_account::get_associated_token_address(&sdk_to_spl(&key), &mint);
...
fn print_help() {
    println!(
        "Usage:
  md-claim <KEYPAIR_PATH> [-u <RPC_URL>]
